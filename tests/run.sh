...
if [ "$QUICK" != "quick" ]; then
  echo "GATE 3 — behaviour (29 suites, real Chromium)"
  for f in addrcopy adddefects bpidesc bulkphoto clearx combo contractorid deep emailattach fixes hdr jobplans loc locmodal pass pdfname pendingcontractors photoedit pvgallery pvphoto pvstable recur rows shop sync tradefilter tradepdf tradereassign wordings; do
