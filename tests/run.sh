...
if [ "$QUICK" != "quick" ]; then
  echo "GATE 3 — behaviour (21 suites, real Chromium)"
  for f in addrcopy adddefects bpidesc bulkphoto combo deep fixes hdr loc pass pdfname pvgallery pvphoto recur rows shop sync tradefilter tradepdf tradereassign wordings; do
