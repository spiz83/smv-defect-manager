...
if [ "$QUICK" != "quick" ]; then
  echo "GATE 3 — behaviour (25 suites, real Chromium)"
  for f in addrcopy adddefects bpidesc bulkphoto clearx combo contractorid deep fixes hdr loc locmodal pass pdfname pendingcontractors pvgallery pvphoto recur rows shop sync tradefilter tradepdf tradereassign wordings; do
