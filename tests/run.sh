...
if [ "$QUICK" != "quick" ]; then
  echo "GATE 3 — behaviour (22 suites, real Chromium)"
  for f in addrcopy adddefects bpidesc bulkphoto combo deep fixes hdr loc pass pdfname pendingcontractors pvgallery pvphoto recur rows shop sync tradefilter tradepdf tradereassign wordings; do
