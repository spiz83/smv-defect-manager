...
if [ "$QUICK" != "quick" ]; then
  echo "GATE 3 — behaviour (24 suites, real Chromium)"
  for f in addrcopy adddefects bpidesc bulkphoto clearx combo contractorid deep fixes hdr loc pass pdfname pendingcontractors pvgallery pvphoto recur rows shop sync tradefilter tradepdf tradereassign wordings; do
