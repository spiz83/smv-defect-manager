...
if [ "$QUICK" != "quick" ]; then
  echo "GATE 3 — behaviour (31 suites, real Chromium)"
  for f in addrcopy adddefects bpidesc bulkphoto clearx combo contractorid deep emailattach fixes hdr jobplans loc locmodal pass pdfname pendingcontractors photoedit pvgallery pvphoto pvstable recur reportindex rows shop sync tempjob tradefilter tradepdf tradereassign wordings; do
