-- Shared defect-wording list for the description autocomplete. (Spiro 2026-08-15)
--
-- Supervisors get suggested defect wordings as they type, narrowed to the
-- trade of whichever contractor they picked. Until now that list was a
-- hardcoded array in index.html, so only a deploy could change it. This table
-- makes it editable from Settings by a manager, and the edit reaches every
-- supervisor on their next sync.
--
-- TARGETS THE LIVE CH TRACKER SCHEMA, not this repo's legacy schema.sql.
-- That file still describes the old workspace/membership model; the live
-- database scopes by profiles.role instead (see cloud-sync.js resolveRole).
-- The policies below therefore gate writes on profiles.role = 'manager'.
--
-- Safe to run against the live database while both apps are in use: a brand
-- new table nothing else references. Until it runs, the phone falls back to
-- its built-in copy of this list, so the app works either way.
--
...
comment on table public.dm_defect_wordings is
    'Defect wordings suggested as a supervisor types, grouped by trade. Editable from the phone app (Settings -> Defect wordings) by managers only.';
...
-- The app only ever reads the live set, ordered within a trade.
...
-- Only MANAGERS write. This is a list every supervisor's suggestions come
-- from, so one person's edit changes what everyone sees.
